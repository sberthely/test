...
def f06(string1, string2):
    ...
    print("'{}' is longer that '{}' by {} chars".format(string2, string1, str2_length - str1_length))
...
#   [1.0, 1.3, 2.443]
def f12():
    fout = open("log_file.txt", "w")

    while True:
        user_input = input('Raw input from user: ')
        try:
            float(user_input)
        except:
            if user_input != 'done':
                # print(user_input)
                # print(fout.write(user_input))
...
# You may need to add two import statements.
...
def f17():
    ...
    list_ = 'SEL'  # Change to your list variable name
